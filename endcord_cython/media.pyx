...
cpdef img_to_term_block_truecolor(
    ...
    cdef int padding_h = (screen_height - img_height // 2) // 2
    cdef int padding_w = (screen_width - img_width) // 2
    cdef int x, y, top_color, bot_color, visible_len
    cdef unsigned char fr, fg, fb, br, bg, bb
    cdef (int, int, int) current_fg, current_bg

    ESC = "\x1b"
    RESET = ESC + "[0m"
    BGR = ESC + "[48;5;" + str(bg_color) + "m"   # bg color is not in r;g;b
    ...
    for y in range(0, img_height - 1, 2):
        ...
        for x in range(img_width):
            ...
            if (fr, fg, fb) != current_fg:   # top pixel
                line_parts.append(ESC)
                line_parts.append("[38;2;")
                line_parts.append(str(fr))
                line_parts.append(";")
                line_parts.append(str(fg))
                line_parts.append(";")
                line_parts.append(str(fb))
                line_parts.append("m")
                current_fg = (fr, fg, fb)
            if (br, bg, bb) != current_bg:   # bot pixel
                line_parts.append(ESC)
